...
class HifiGANGenerator(torch.nn.Module):
    """Generate waveforms from input features such as spectrogram, latents, and so on."""
    def __init__(
        self
        in_channels:int,
        out_channels:int=1,
        ...
        for i, (upsample_rate, upsample_kernel_size) in enumerate(zip(upsample_rates, upsample_kernel_sizes)):
            self.layers.append(
        ...
        for i in range(len(self.layers)):
            ...
            for resblock_kernel_size, resblock_dilation_size in zip(resblock_kernel_sizes, resblock_dilation_sizes):
                self.resblocks.append(
                    ResBlock1(
                        ch, resblock_kernel_size, resblock_dilation_size
                    )
                )
