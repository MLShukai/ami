...
def get_2d_sincos_positional_embeddings_from_grid(embed_dim: int, grid: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    assert embed_dim % 2 == 0

    # use half of dimensions to encode grid_h
    embeddings_h = get_1d_sincos_positional_embeddings_from_grid(embed_dim // 2, grid[0])  # [grid_size_h*grid_size_w, embed_dim//2]
    embeddings_w = get_1d_sincos_positional_embeddings_from_grid(embed_dim // 2, grid[1])  # [grid_size_h*grid_size_w, embed_dim//2]

    embeddings = np.concatenate([embeddings_h, embeddings_w], axis=1)  # [grid_size_h*grid_size_w, embed_dim]
    return embeddings


def get_1d_sincos_positional_embeddings_from_grid(embed_dim: int, positions: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    """
    Args:
        embed_dim (int): dim of positional embeddings.
    ...
    positional_embeddings = np.concatenate([positional_embeddings_sin, positional_embeddings_cos], axis=1)  # [grid_size_h*grid_size_w, embed_dim]
    return positional_embeddings
